...
public class DashboardComponent
{
    ...
    public async Task<DashboardDataDto> GetDashboardDataAsync()
    {
        ...
        try
        {
            ...
            string summaryQuery = @"
                SELECT 
                    ...
                    (SELECT COUNT(1) FROM WorkflowExecutionSteps wes JOIN WorkflowExecutions we ON we.Id = wes.WorkflowExecutionId 
                        WHERE we.CompanyId = @CompanyId AND wes.AssignedUserId = @UserId AND wes.IsActive = TRUE AND wes.Decision IS NULL) AS PendingApprovals,
                    (SELECT COUNT(1) FROM DocumentUserTraining WHERE CompanyId = @CompanyId AND EmployeeCode = @UserId AND TrainingStatus = 0 AND IsDeleted = FALSE) AS PendingTrainings,
            ...
            var summary = await _common.QueryFirstOrDefaultAsync<DashboardSummaryDto>(summaryQuery, new { CompanyId = companyId, UserId = userId });
            if (summary != null)
            {
            ...
            string distributionQuery = @"
                ...
                LEFT JOIN Documents d ON d.DocumentTypeCode = dt.Code AND d.CompanyId = @CompanyId AND d.CreatedBy = @UserId AND d.IsDeleted = FALSE
            ...
            var tasks = await _common.QueryAsync<DashboardPendingTaskDto>(tasksQuery, new { CompanyId = companyId, UserId = userId });
            ...
            string auditQuery = @"
                SELECT 
                    ...
                    UserId::text AS User
                FROM AuditLogs
                WHERE CompanyId = @CompanyId AND UserId::text = @UserId
                ORDER BY Timestamp DESC
                LIMIT 10;";
            
            var activities = await _common.QueryAsync<RecentActivityDto>(auditQuery, new { CompanyId = companyId, UserId = userId });
