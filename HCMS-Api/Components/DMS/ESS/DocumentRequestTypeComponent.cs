...
public class DocumentRequestTypeComponent
{
    ...
    public DocumentRequestTypeComponent(
    ...
    public async Task<DocumentRequestTypeReadDto> CreateAsync(DocumentRequestTypeCreateDto input)
    {
        try
        {
            ...
            if (lastCodeObj != null)
            {
                ...
                var numericPart = lastCode.Replace("DRT-", "");

                if (int.TryParse(numericPart, out int lastNumber))
                    nextNumber = lastNumber + 1;
            }

            string generatedCode = $"DRT-{nextNumber:D4}";

            // 🧾 Insert
            string insertQuery = $@"
                    INSERT INTO DocumentRequestTypes
                    (   CompanyId,
                        Code,
                        Name,
                        Description,
    ...
    public async Task<PaginationResult<DocumentRequestTypeReadDto>> GetAllAsync(TableFiltersDto input)
    {
        try
        {
            string _CompanyId = _utilities.GetCompanyId(_clientContextService.GetClientIP());
            ...
            {
                return new PaginationResult<DocumentRequestTypeReadDto>
                {
                    Items = new List<DocumentRequestTypeReadDto>(),
                    TotalCount = 0
                };
            }
    ...
    public async Task<DocumentRequestTypeReadDto> GetByCodeAsync(string code)
    {
        try
        {
            string _CompanyId = _utilities.GetCompanyId(_clientContextService.GetClientIP());
    ...
    public async Task<DocumentRequestTypeReadDto> UpdateAsync(DocumentRequestTypeUpdateDto input)
    {
        try
        {
            string _CompanyId = _utilities.GetCompanyId(_clientContextService.GetClientIP());
